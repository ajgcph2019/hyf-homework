...
const moviesWithShortTitle = movies.filter((value) => value.title.length < 5);
console.log(moviesWithShortTitle);

// 2. Create an array of movie titles with long movie titles

const moviesWithLongTitle = movies.filter((values) => values.title.length > 30);
...
const extraKeyNamedTag = movies.map((movie) => {
  if (movie.rating >= 7) {
    movie.tag = `Good`;
    return movie;
...
console.log(extraKeyNamedTag);

// 5. chaining, movies rated higher than 6

const moviesRatedHigherThanSix = movies
...
movies
  ...
  .forEach((title) =>
    title.filter((word, index) =>
      title.indexOf(word) !== index ? duplicatedWordsInTitles.push(word) : false
    )
  );
...
movies
  ...
  .forEach((title) => {
    title.split(" ").forEach((word) => {
      const position = wordCount
        .map((e) => e.word.toLowerCase())
        .indexOf(word.toLowerCase());
      if (position !== -1) {
        wordCount[position].count += 1;
      } else {
        wordCount.push({
...
const averageRating = movies
  .reduce((acc, movie) => (acc += movie.rating / movies.length), 0)
  .toFixed(2);
